...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    while current_day <= days_in_simulation:
        ...
        total_earning += (current_day - last_bribe_day) * current_earning
        current_money += (current_day - last_bribe_day) * current_earning
        if plot_type == STANDARD:
            days_vs_earnings.append((current_day, total_earning))
        elif plot_type == LOGLOG:
            days_vs_earnings.append((math.log(current_day), math.log(total_earning)))
        # check whether we have enough money to bribe without waiting
        while current_money >= bribe:
            current_earning += 100
            current_money -= bribe
            bribe += bribe_cost_increment
            last_bribe_day = current_day
        # advance current_day to day of next bribe (DO NOT INCREMENT BY ONE DAY)
        current_day += int(math.ceil(float(bribe - current_money) / current_earning))
