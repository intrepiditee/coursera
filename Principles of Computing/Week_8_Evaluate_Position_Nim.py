...
def evaluate_position(current_num):
    ...
    if current_num % 4 == 0:
        ...
        if evaluate_position(current_num - current_num % 4) == 'lost':
            return 'won'
...
def evaluate_memo_position(current_num, memo_dict):
    ...
    global counter
    counter += 1
    for removed in range(1, min(4, current_num + 1)):
        next_num = current_num - removed
        if next_num not in memo_dict:
            memo_dict[next_num] = evaluate_memo_position(next_num, memo_dict)
        if memo_dict[next_num] == 'lost':
            return 'won'
    return 'lost'
